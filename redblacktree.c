...
static struct rb_node *tree_search(struct rb_node*, char *);
...
void tree_insert(struct redblack_tree *T, char *key)
{
...
dump_node(struct rb_node *x)
{
	if (x == NIL)
		printf("NIL ");
	printf("%p { %s, %s, ", x, (x->color == BLACK) ? "BLACK" : "RED", x->key);
...
struct rb_node *tree_find(struct redblack_tree *tree, char *key)
{
	return tree_search(tree->root, key);
}

static struct rb_node *tree_search(struct rb_node *x, char *key)
{
	for ( ; x != nil && strcmp(x->key, key) != 0; x = strcmp(key, x->key) < 0 ? x->left : x->right)
		;
	return x;
}
...
{
	/* Declare node */
	fprintf(fp, "    n%s [label=\"%s\",color=%s,fontcolor=white,style=filled, shape=circle, width=0.5, fixedsize=true,fontname=Palatino];\n",
